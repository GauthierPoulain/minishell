...
static void	quotes_job(t_ptoken *token)
{
	...
	{
		if ((token + 1)->spaces > 0)
			(token + 1)->str = ft_strjoin(ft_calloc_char(
						(token + 1)->spaces + 1, ' '), (token + 1)->str);
		if ((token + 1)->spaces_after > 0)
...
{
	quotes_job(token);
	if ((token + 1)->skip_join)
		return (false);
	else if ((token + 1)->str && (((token)->is_in_quotes
...
{
	if (!(argv + *i)->is_in_quotes
		&& !(argv + *i)->is_in_squotes && (argv + *i)->spaces > 0)
